...
// RPC is a struct that represents a remote procedure call client
type RPC struct {
...
// NewRPC initializes the RPC client with the logger, IO wrapper, RPC URL, API key, max retries, retry delay, and report retry attempts
...
// Request sends an RPC request to the server with the specified method and parameters
